...
PRICE_REQUEST_KEYWORDS = ["средний чек", "сколько стоит", "сколько стоят", "цены", "ценник", "бюджет", "во сколько обойдётся", "сколько потрачу"]
...
- Дополняй список своими знаниями о Москве — добавляй известные заведения которые знаешь хорошо: Белуга, White Rabbit, Delicatessen, Пушкин, Uilliam's и другие топовые места
- Для каждого места из своих знаний пиши только то в чём уверен: название, адрес если знаешь точно, метро, рейтинг если знаешь реальную цифру
- Если не уверен в адресе — напиши только название и район или метро
- Если не уверен в рейтинге — не пиши его
- Никогда не придумывай детали которых не знаешь
- Рейтинг и часы работы — всегда указывай если есть в данных
- Средний чек — указывай ТОЛЬКО если пользователь сам спросил о ценах или бюджете
...
- Учитывай день недели и время при выборе мест
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    price_note = "Пользователь спросил о ценах — укажи примерный ценник для каждого места." if show_price else "Средний чек не указывай — пользователь не спрашивал."
