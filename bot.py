...
PRICE_REQUEST_KEYWORDS = ["средний чек", "сколько стоит", "цены", "ценник", "сколько потрачу", "по деньгам", "что по деньгам", "сколько денег", "во сколько обойдётся", "сколько выйдет"]

FOLLOWUP_KEYWORDS = ["а что", "а сколько", "а как", "расскажи подробнее", "по деньгам", "по ценам", "уточни", "поподробнее", "что насчёт", "а если"]
...
def is_followup(text):
    """Уточняющий вопрос про текущий план — не новый запрос"""
    t = text.lower().strip()
...
- Грамотность — абсолютный приоритет. Никаких орфографических, пунктуационных или грамматических ошибок. Перепроверяй каждое слово перед ответом.
...
- Если пользователь задаёт уточняющий вопрос про текущий план ("а что по деньгам", "расскажи подробнее", "а как добраться") — отвечай на вопрос не создавая новый план с нуля. Дополняй уже предложенные места новой информацией.
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Показываем постоянную панель кнопок
    await update.message.reply_text(
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Обрабатываем постоянные кнопки
    if text == "🏙 Новый вечер":
        user_histories[user_id] = []
        user_last_request[user_id] = ""
        user_district[user_id] = None
        user_format[user_id] = None
        await update.message.reply_text(
            "Начнём заново. Какой вечер планируем?",
            reply_markup=InlineKeyboardMarkup(FORMAT_BUTTONS)
        )
        return
    ...
    if is_repeat_request(text):
        ...
        places = search_places(user_last_request.get(user_id, text), district=district)
        note = "ВАЖНО: уточняющий вопрос про уже предложенный план. Не создавай новый план. Просто ответь на вопрос — добавь запрошенную информацию к уже предложенным местам."
    else:
